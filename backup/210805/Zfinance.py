...
###################
        Min15HistoryData = self.symbol.history(interval='1d',period='max',proxy=PROXYEN,prepost=True)

        StartTimestamp = time.time() - 10000*24*60*60
        timeArray = time.localtime(StartTimestamp)
        StartTimestring = time.strftime("%Y-%m-%d", timeArray)
        Min15HistoryData = self.symbol.history(interval='1h',period='1y',proxy=PROXYEN,prepost=True)

        StartTimestamp = time.time() - 10000*24*60*60
        timeArray = time.localtime(StartTimestamp)
        StartTimestring = time.strftime("%Y-%m-%d", timeArray)
        Min15HistoryData = self.symbol.history(interval='30m',period='ytd',proxy=PROXYEN,prepost=True)
        ...
        timeArray = time.localtime(StartTimestamp)
        StartTimestring = time.strftime("%Y-%m-%d", timeArray)
        Min15HistoryData = self.symbol.history(interval='5m',start=StartTimestring,proxy=PROXYEN,prepost=True)

        StartTimestamp = time.time() - 6*24*60*60
        timeArray = time.localtime(StartTimestamp)
        StartTimestring = time.strftime("%Y-%m-%d", timeArray)
        Min15HistoryData = self.symbol.history(interval='1m',start=StartTimestring,proxy=PROXYEN,prepost=True)

        StartTimestamp = time.time() - 10000*24*60*60
        timeArray = time.localtime(StartTimestamp)
        StartTimestring = time.strftime("%Y-%m-%d", timeArray)
        Min15HistoryData = self.symbol.history(interval='90m',start=StartTimestring,proxy=PROXYEN,prepost=True)
###################
